...
@router.delete("/complaints/{complaint_id}/", dependencies = [Depends(oauth2_scheme), Depends(is_admin)], status_code = 202)
async def delete_complaint(complaint_id: int):
    return await ComplaintManager.delete_complaint(complaint_id)


@router.put("/complaints/{complaint_id}/approve",  dependencies = [Depends(oauth2_scheme), Depends(is_approver)], status_code = 202)
async def approve_complaint(complaint_id: int):
    return await ComplaintManager.approve_complaint(complaint_id)


@router.put("/complaints/{complaint_id}/reject",   dependencies = [Depends(oauth2_scheme), Depends(is_approver)], status_code = 202)
